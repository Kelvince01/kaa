...
import { genCodeChallenge } from "./helpers.util";
...
export const genTotp = async (secret: string): Promise<string> => {
  ...
  const offset = hashArray.at(-1)! & 0xf;
...
export const bcryptText = (text: string) => {
  ...
  const hash = Bun.password.hashSync(text, "bcrypt");
  return hash;
};

export const bcryptCompare = (text: string, hash: string) => {
  // biome-ignore lint/correctness/noUndeclaredVariables: false positive
  const isMatch = Bun.password.verifySync(text, hash, "bcrypt");
  return isMatch;
};
...
export const getPasskeyRpId = () => {
  const authServerUrl = config.app.url; // process.env.AUTH_SERVER_URL;
  if (!authServerUrl) throw new Error("AUTH_SERVER_URL is not defined");
