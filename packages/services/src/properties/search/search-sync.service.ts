...
import type { IProperty, IContractor } from "@kaa/models/types";
import type mongoose from "mongoose";

/**
 * Service to sync MongoDB changes with Elasticsearch
 */
class SearchSyncService {
	/**
	 * Sync property to Elasticsearch when created or updated
	 */
	async syncProperty(
		property: IProperty,
		...
		try {
			...
			logger.error(`Failed to sync property ${property._id} to search index:`, error);
			// Don't throw error to avoid breaking the main operation
		}
	}

	/**
	 * Sync contractor to Elasticsearch when created or updated
	 */
	async syncContractor(
		...
		try {
			...
			logger.error(`Failed to sync contractor ${contractor._id} to search index:`, error);
			// Don't throw error to avoid breaking the main operation
		}
	}
	...
	async batchSyncProperties(properties: IProperty[]): Promise<void> {
		try {
			...
			await elasticsearchService.bulkIndex("properties", documents);
			logger.info(`Batch synced ${properties.length} properties to search index`);
		} catch (error) {
	...
	async batchSyncContractors(contractors: IContractor[]): Promise<void> {
		try {
			...
			await elasticsearchService.bulkIndex("contractors", documents);
			logger.info(`Batch synced ${contractors.length} contractors to search index`);
		} catch (error) {
