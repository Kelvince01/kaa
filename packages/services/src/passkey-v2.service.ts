...
export const parseAndValidatePasskeyAttestation = (
  ...
  if (
    !authenticatorData.verifyRelyingPartyIdHash(
      config.mode === "development" ? "localhost" : config.domain
    )
  ) {
  ...
  if (encodeBase64(clientData.challenge) !== challengeFromCookie)
    throw new Error("Invalid challenge");

  // Use "http://localhost:PORT" for localhost
  if (clientData.origin !== config.frontendUrl)
 ...
 */
export const verifyPassKeyPublic = (
  signature: string,
