...
export const getUsersWithRole = async (
  ...
  return {
    users: userRoles.map((ur) => ({
      ...
      isVerified: !!(ur.userId as any)?.verification?.emailVerifiedAt,
      phoneVerified: !!(ur.userId as any)?.verification?.phoneVerifiedAt,
      identityVerified: !!(ur.userId as any)?.verification?.identityVerifiedAt,
