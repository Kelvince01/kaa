...
export const Link = TiptapLink.extend<LinkOptions, LinkEditorStorage>({
  ...
  onSelectionUpdate() {
    if (this.storage.mode === -1 && !this.editor.isActive("link")) {
      (this.editor.commands as any).confirmEditLink();
    }
  },
      ...
      new Plugin({
        ...
        appendTransaction: (transactions, oldState, newState) => {
          ...
          const range = getMarkRange(
            ...
            newState.schema.marks.link as any
          );
          if (!range) {
            return;
          }
  ...
  addKeyboardShortcuts() {
    return {
      "Mod-k": () => (this.editor.commands as any).startEditLink(),
    };
  },
});
