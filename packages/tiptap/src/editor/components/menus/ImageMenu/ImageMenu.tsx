...
export const ImageMenu = () => {
  ...
  const image = useEditorState({
    ...
    selector: (ctx) => {
      if (
        !(ctx.editor.isActive("image") || ctx.editor.isActive("imageFigure"))
      ) {
        return null;
      }
  ...
  const removeImage = () => (editor.chain().focus() as any).removeImage().run();
    ...
    >
      {isEditText ? (
        ...
        <Toolbar>
          ...
          />
          <ToolbarDivider />
          <SizeDropdown onChange={setSize} value={image?.width} />
          <ToolbarDivider />
          <MenuButton
            icon="Download"
            ...
            tooltip="Download"
          />
          <MenuButton icon="Trash" onClick={removeImage} tooltip="Delete" />
        </Toolbar>
      )}
    </BubbleMenu>
  );
