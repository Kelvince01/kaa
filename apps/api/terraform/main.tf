  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "MONGODB_READ_URI", value = var.mongodb_read_uri },
        { name = "MONGODB_POOL_SIZE", value = tostring(var.mongodb_pool_size) },
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget --no-verbose --tries=1 --spider http://localhost:5000/api/monitoring/health || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
...
# IAM policy attachment for ECS tasks
