  ...
  app
    ...
    .get(
      ...
      async ({ set, query }) => {
        try {
          ...
          // Price filters
          if (query.minRent)
            filters.minRent = Number.parseInt(query.minRent, 10);
          if (query.maxRent)
            filters.maxRent = Number.parseInt(query.maxRent, 10);
    ...
    .get(
      ...
      async ({ set, query }) => {
        try {
          ...
          const radius = query.radius
            ? Number.parseInt(query.radius, 10)
            : 5000;
          const limit = query.limit ? Number.parseInt(query.limit, 10) : 10;
      ...
      },
      {
        ...
        detail: {
          ...
          description:
            "Get properties near specified coordinates within radius",
          tags: ["properties"],
        },
      }
    )
    ...
    .get(
      ...
      },
      {
        ...
        query: t.Object({
          radius: t.Optional(
            t.Number({ minimum: 0.1, maximum: 10, default: 2 })
          ),
        }),
        response: {
          200: t.Object({
