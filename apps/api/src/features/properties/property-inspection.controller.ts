...
export const propertyInspectionController = new Elysia({
  detail: {
    tags: ["inspections"],
    summary: "Property inspections",
    description: "Property inspections endpoints",
    security: [{ bearerAuth: [] }],
  },
}).group("/properties/inspections", (app) =>
  app
    .use(authPlugin)
    .post(
      "/",
      async ({ body, set, user }) => {
        try {
          ...
          await inspection.populate("tenant", "firstName lastName email phone");
    ...
    .patch(
      ...
      async ({ params, body, set, user }) => {
        try {
          ...
          const updatedInspection = await PropertyInspection.findByIdAndUpdate(
            inspectionId,
            { $set: updateData },
            { new: true }
          )
