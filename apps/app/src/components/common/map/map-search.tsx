...
import { iconMap, type LocationFeature, type LocationSuggestion } from "@/lib/mapbox/utils";
...
export default function MapSearch() {
			...
			try {
				const res = await fetch(
					`https://api.mapbox.com/search/searchbox/v1/suggest?q=${encodeURIComponent(
						debouncedQuery
					)}&access_token=${process.env.NEXT_PUBLIC_MAPBOX_TOKEN}&session_token=${
	...
	return (
		<>
			<section className="-translate-x-1/2 absolute top-4 left-1/2 z-10 w-[90vw] rounded-lg shadow-lg sm:left-4 sm:w-[350px] sm:translate-x-0">
				<Command className="rounded-lg">
					...
					>
						...
						{isSearching && <Loader2 className="size-4 shrink-0 animate-spin text-primary" />}
					</div>
										...
										>
											<div className="flex items-center space-x-2">
												...
												<div className="flex flex-col">
													<span className="max-w-[270px] truncate font-medium text-sm">
														{location.name}
													</span>
													<span className="max-w-[270px] truncate text-muted-foreground text-xs">
			...
			{selectedLocation && (
				<LocationPopup location={selectedLocation} onClose={() => setSelectedLocation(null)} />
			)}
		</>
	);
}
