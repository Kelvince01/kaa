...
class Observer {
  subscribers: Array<
    (dialog: DialogT | DialogToRemove | DialogToReset) => void
  >;
  ...
  subscribe = (
    subscriber: (dialog: DialogT | DialogToRemove | DialogToReset) => void
  ) => {
  ...
  set = (data: DialogT) => {
    ...
    const existingDialogIndex = this.dialogs.findIndex(
      (dialog) => dialog.id === data.id
    );
  ...
  get = (id: number | string) =>
    this.dialogs.find((dialog) => dialog.id === id);
  ...
  update = (id: number | string, data: Partial<DialogT>) => {
    ...
    this.dialogs[existingDialogIndex] = updatedDialog as DialogT;
    this.publish(updatedDialog as DialogT);
  };
  ...
  reset = (id?: number | string) => {
    ...
    // Reset all dialogs
    for (const dialog of this.dialogs)
      this.publish({ id: dialog.id, reset: true });
  };
}
