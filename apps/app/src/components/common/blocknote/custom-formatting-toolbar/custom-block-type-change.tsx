...
import { customBlockTypeSelectItems } from "@/components/common/blocknote/blocknote-config";
import type { BasicBlockTypes } from "@/components/common/blocknote/types";

export const KaaCustomBlockTypeSelect = () => {
  ...
  const filteredItems = useMemo(
    () =>
      blockTypeSelectItems(dict).filter((item) =>
        customBlockTypeSelectItems.includes(item.type as BasicBlockTypes)
      ),
    [dict]
  );
    ...
    if (currentBlock?.props?.level) {
      return filteredItems.find(
        (el) =>
          ...
          el.name.includes(currentBlock.props.level ?? "")
      );
    }
  ...
  return (
    <Components.Generic.Menu.Root>
      ...
      <Components.Generic.Menu.Dropdown
        className="bn-shadcn bn-menu-dropdown"
