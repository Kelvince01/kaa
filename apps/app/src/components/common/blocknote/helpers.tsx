...
export const getSortedSlashMenuItems = (
  items: DefaultReactSuggestionItem[],
  allowedBlockTypes: (KaaCustomBlockTypes | BasicBlockTypes)[]
) => {
  ...
  const slashMenuIndexed = indexedItems.filter((i) =>
    isAllowedSlashMenu(i, allowedBlockTypes)
  );
  const slashMenuNotIndexed = notIndexedItems.filter((i) =>
    isAllowedSlashMenu(i, allowedBlockTypes)
  );
  ...
  const blocksStringifyContent = blocks
    .map((block) => {
      if (Array.isArray(block.content))
        return (block.content[0] as { text: string } | undefined)?.text;
      return block.type;
    })
    .join("");
...
export const compareIsContentSame = async (
...
export const handleSubmitOnEnter = (
  editor: CustomBlockNoteSchema
): CustomBlockNoteSchema["document"] | null => {
  ...
  if (Array.isArray(lastBlock?.content)) {
    ...
    if (lastBlockContent.length > 0)
      (lastBlockContent[0] as any).text =
        // biome-ignore lint/performance/useTopLevelRegex: required by author
        lastBlockContent[0]?.text.replace(/\n$/, "") ?? ""; // Remove the last newline character
    const updatedLastBlock = { ...lastBlock, content: lastBlockContent };
    return [
      ...blocks.slice(0, -1),
      updatedLastBlock,
    ] as CustomBlockNoteSchema["document"];
  }
  return null;
};

export const updateSourcesFromDataUrl = (
  elementId: string,
  ...
  const onElClick = (e: MouseEvent) => {
    ...
    const slideNum = attachments.findIndex(
      ({ url }) => url === target.currentSrc
    );
