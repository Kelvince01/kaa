...
class SheetsStateObserver {
  sheets: SheetT[] = []; // Array to store the current sheets
  ...
  private readonly notifySubscribers = (action: SheetAction & SheetT) => {
  ...
  remove = (id?: string, excludeId?: string) => {
    ...
    const sheetstoRemove = this.sheets.filter(
      (sheet) => sheet.id !== excludeId
    );
    for (const sheet of sheetstoRemove)
      this.notifySubscribers({ id: sheet.id, remove: true });
    this.sheets = excludeId
      ? this.sheets.filter((sheet) => sheet.id === excludeId)
      : [];
  };
  ...
  create = (content: React.ReactNode, data?: Omit<SheetT, "content">) => {
    // biome-ignore lint/complexity/useSimplifiedLogicExpression: use simplified logic expression
