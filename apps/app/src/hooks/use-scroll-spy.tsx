...
export const useScrollSpy = ({ sectionIds = [] }: { sectionIds: string[] }) => {
  ...
  const entriesRef = useRef<IntersectionEntry[]>(
    sectionIds.map((id) => ({ id, ratio: 0 }))
  ); // Map to store id -> intersectionRatio
  ...
  useEffect(() => {
    ...
    observer.current = new IntersectionObserver((observedEntries) => {
      ...
      if (fullyIntersecting.length === 1) {
        ...
        const mostProminent = entriesArray.reduce((prev, curr) =>
