...
const sortedBreakpoints = Object.keys(breakpoints).sort(
  (a, b) =>
    Number.parseInt(breakpoints[a] || "0", 10) -
    Number.parseInt(breakpoints[b] || "0", 10)
);
...
const getMatchedBreakpoints = () => {
  ...
  for (let i = 1; i < sortedBreakpoints.length; i++) {
    const prevBreakpointSize = Number.parseInt(
      breakpoints[(sortedBreakpoints as string[])[i - 1] as string] || "0",
      10
    );
    const currentBreakpointSize = Number.parseInt(
      breakpoints[sortedBreakpoints[i] as string] || "0",
      10
    );
