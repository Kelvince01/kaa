...
export const amenityKeys = {
  ...
  nearby: (query: NearbyAmenitiesQuery) =>
    [...amenityKeys.all, "nearby", query] as const,
  ...
  search: (query: string, filters?: any) =>
    [...amenityKeys.all, "search", query, filters] as const,
  ...
  approvalStats: (county?: string) =>
    [...amenityKeys.all, "approval-stats", county] as const,
  metadata: () => [...amenityKeys.all, "metadata"] as const,
  autoPopulationStatus: () =>
    [...amenityKeys.all, "auto-population", "status"] as const,
  autoDiscoveryStats: (county?: string) =>
    [...amenityKeys.all, "auto-discovery-stats", county] as const,
  dataValidation: (county?: string) =>
    [...amenityKeys.all, "data-validation", county] as const,
...
export const useAmenityScore = (
  ...
  return useQuery({
    ...
    queryFn: () =>
      AmenityService.calculateAmenityScore(latitude, longitude, radius),
    enabled: !!(latitude && longitude),
...
export const useCreateAmenity = () => {
  ...
  return useMutation({
    mutationFn: (amenityData: CreateAmenityRequest) =>
      AmenityService.createAmenity(amenityData),
    onSuccess: () => {
...
export const useApproveAmenity = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || "Failed to approve amenity"
      );
    },
  });
};
...
export const useBulkApproveAmenities = () => {
  ...
  return useMutation({
    mutationFn: (amenityIds: string[]) =>
      AmenityService.bulkApproveAmenities(amenityIds),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: amenityKeys.pending() });
      queryClient.invalidateQueries({ queryKey: amenityKeys.approvalStats() });
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || "Failed to bulk approve amenities"
      );
    },
  });
};
...
export const useDiscoverAmenities = () => {
  ...
  return useMutation({
    mutationFn: (request: DiscoveryRequest) =>
      AmenityService.discoverAmenities(request),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: amenityKeys.all });
      toast.success(
        `Discovered ${data.discovered.length} amenities, saved ${data.saved}`
      );
    },
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || "Failed to discover amenities"
      );
    },
  });
};
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: amenityKeys.property(variables.propertyId),
      });
      queryClient.invalidateQueries({ queryKey: amenityKeys.pending() });
      toast.success(
        `Discovered ${data.discovered.length} amenities for property`
      );
    },
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message ||
          "Failed to discover property amenities"
      );
    },
  });
};
...
export const useDiscoverMissingAmenities = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || "Failed to discover missing amenities"
      );
    },
  });
};
...
export const useVerifyAmenityWithLevel = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      AmenityService.verifyAmenityWithLevel(
        amenityId,
        verificationLevel,
        notes
      ),
...
export const useAmenitiesByDiscoveryStatus = (
  ...
  useQuery({
    ...
    queryFn: () =>
      AmenityService.getAmenitiesByDiscoveryStatus(isAutoDiscovered, options),
    staleTime: 5 * 60 * 1000,
  });
