...
interface AmenityManagementDashboardProps {
	county?: string;
}

const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042", "#8884D8"];

export function AmenityManagementDashboard({ county }: AmenityManagementDashboardProps) {
	...
	const sourceChartData = autoDiscoveryStats
		? Object.entries(autoDiscoveryStats.sourceBreakdown).map(([source, count]) => ({
	...
	const categoryChartData = autoDiscoveryStats
		? Object.entries(autoDiscoveryStats.categoryCounts).map(([category, count]) => ({
		...
		<div className="space-y-6">
					...
					<Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
						...
						<DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
							<DialogHeader>
								...
								<DialogDescription>Manually add a new amenity to the database</DialogDescription>
							</DialogHeader>
							<CreateAmenityForm
			...
			{autoPopulationStatus && (
				<Alert>
					...
					<AlertDescription>
						Auto-discovery service: {autoPopulationStatus.isProcessing ? "Processing" : "Idle"}
			...
			<Tabs value={activeTab} onValueChange={setActiveTab}>
				...
				<TabsContent value="overview" className="space-y-6">
						...
						<Card>
							<CardContent className="p-6">
								<div className="flex items-center space-x-2">
									...
									<div>
										<p className="font-medium text-muted-foreground text-sm">Pending Approval</p>
										<p className="font-bold text-2xl">{approvalStats?.pending || 0}</p>
									</div>
								</div>
							</CardContent>
						</Card>
						...
						<Card>
							<CardContent className="p-6">
								<div className="flex items-center space-x-2">
									...
									<div>
										<p className="font-medium text-muted-foreground text-sm">Total Approved</p>
										<p className="font-bold text-2xl">{approvalStats?.approved || 0}</p>
									</div>
								</div>
							</CardContent>
						</Card>
						...
						<Card>
							<CardContent className="p-6">
								<div className="flex items-center space-x-2">
									...
									<div>
										<p className="font-medium text-muted-foreground text-sm">Auto-Discovered</p>
						...
						<Card>
							<CardContent className="p-6">
								<div className="flex items-center space-x-2">
									...
									<div>
										<p className="font-medium text-muted-foreground text-sm">Verification Rate</p>
										<p className="font-bold text-2xl">
					...
					{/* Data Quality Alerts */}
					{dataValidation?.suggestions && dataValidation.suggestions.length > 0 && (
						<Card>
							<CardHeader>
								...
								<div className="space-y-4">
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											...
											<Badge variant={autoPopulationStatus?.isProcessing ? "default" : "outline"}>
							...
							<CardContent>
								<div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-4">
									<div className="text-center">
										<div className="font-bold text-2xl">{dataValidation.totalAmenities}</div>
									...
									<div className="text-center">
										...
										<div className="text-muted-foreground text-xs">Unverified</div>
									</div>
									<div className="text-center">
										<div className="font-bold text-2xl text-orange-600">
											{dataValidation.missingContactCount}
										</div>
										<div className="text-muted-foreground text-xs">Missing Contact</div>
									</div>
									<div className="text-center">
										<div className="font-bold text-2xl text-red-600">
											{dataValidation.duplicatesCount}
										</div>
										<div className="text-muted-foreground text-xs">Potential Duplicates</div>
									</div>
								</div>
