...
export function VerificationDialog({
	...
	const levelInfo = verificationLevels.find((level) => level.value === selectedLevel);
			...
			<DialogContent className="sm:max-w-md">
				...
				<div className="mb-4 rounded-lg bg-muted/50 p-4">
					...
					<div className="flex flex-wrap gap-1">
						<Badge variant={amenity.isAutoDiscovered ? "secondary" : "default"} className="text-xs">
							{amenity.isAutoDiscovered ? "Auto-Discovered" : "Manual Entry"}
						</Badge>
						{amenity.isAutoDiscovered && (
							<Badge variant="outline" className="text-xs">
								{amenity.source.includes("google") ? "Google Places" : "OpenStreetMap"}
							</Badge>
						)}
					</div>
				</div>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>Document what was verified or any issues found</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
