...
class LocationService {
  ...
  async validateAddress(
    ...
    try {
      const response = await httpClient.api.post(`${this.baseUrl}/validate`, {
        address,
      });
  ...
  async getCountySuggestions(query: string, limit?: number): Promise<string[]> {
    try {
      ...
      const response = await httpClient.api.get(
        `${this.baseUrl}/autocomplete/counties?${params}`
      );
  ...
  async getNeighborhoodSuggestions(
    ...
    try {
      ...
      const response = await httpClient.api.get(
        `${this.baseUrl}/autocomplete/neighborhoods?${params}`
      );
  ...
  parseAddress(formattedAddress: string): Partial<Address> {
    ...
    // biome-ignore lint/performance/useTopLevelRegex: use a top level regex fix required
