...
const addressSchema = z.object({
	street: z.string().min(3, "Street address must be at least 3 characters").max(200),
...
const pricingSchema = z
	.object({
		...
		maintenanceFee: z.number().min(0, "Maintenance fee cannot be negative").optional(),
		utilitiesCost: z.number().min(0, "Utilities cost cannot be negative").optional(),
...
const basicInfoSchema = z
	.object({
		...
		availableFrom: z.date().min(new Date(), "Available date cannot be in the past"),
...
const detailsSchema = z
	.object({
		...
		orientation: z
			...
			.optional(),
		view: z.array(z.enum(["city", "ocean", "mountain", "garden", "pool", "courtyard", "street"])),
		parking: z
			.object({
				spaces: z
					.number()
				...
				type: z
					.enum(["garage", "carport", "street", "driveway", "covered", "uncovered"])
					.optional(),
...
const contactSchema = z.object({
	...
	showingInstructions: z.string().max(500, "Instructions cannot exceed 500 characters").optional(),
	responseTime: z.enum(["immediate", "within_hour", "within_day", "within_week"]),
	languages: z.array(z.enum(["english", "swahili", "french", "arabic", "spanish"])),
});
...
export const getStepProgress = (stepId: keyof typeof stepSchemas, data: any) => {
