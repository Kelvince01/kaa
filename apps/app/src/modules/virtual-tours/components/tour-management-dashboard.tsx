...
import { TourStatus, TourType, type VirtualTour } from "../virtual-tour.type";
import CreateTourForm from "./create-tour-form";
import VirtualTourViewer from "./virtual-tour-viewer";

type TourManagementDashboardProps = {
  propertyId: string;
};

export const TourManagementDashboard: React.FC<
  TourManagementDashboardProps
> = ({ propertyId }) => {
  ...
  const handleTourAction = async (action: string, tour: VirtualTour) => {
    switch (action) {
      ...
      case "smart-connections":
        await generateConnectionsMutation.mutateAsync(tour.id);
        break;
      case "share":
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage immersive virtual tours for your property
          </p>
        </div>
      ...
      {/* Tours Grid */}
      <Tabs className="space-y-4" defaultValue="grid">
        <TabsList>
...
const TourCard: React.FC<TourCardProps> = ({
  ...
  const { data: analytics } = useTourAnalytics(
    tour.id,
      ...
      <div className="relative aspect-video overflow-hidden rounded-t-lg bg-gray-100">
        ...
        <div className="absolute inset-0 bg-black/0 transition-colors group-hover:bg-black/20">
          <div className="absolute top-2 left-2">
            <Badge className="bg-black/80 text-white" variant="secondary">
              {getTourTypeIcon(tour.type)}
          ...
          <div className="absolute top-2 right-2">
            <Badge variant={getStatusColor(tour.status as TourStatus)}>
              {tour.status}
            </Badge>
          </div>
...
const TourAnalyticsDashboard: React.FC<TourAnalyticsDashboardProps> = ({
  ...
  const totalViews = tours.reduce(
    (sum, tour) => sum + tour.analytics.totalViews,
    0
  );
  const totalTours = tours.length;
  const avgDuration =
    tours.reduce((sum, tour) => sum + tour.analytics.averageDuration, 0) /
      tours.length || 0;
  const avgCompletionRate =
    tours.reduce((sum, tour) => sum + tour.analytics.completionRate, 0) /
      tours.length || 0;
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-bold text-2xl">
                {Math.round(avgDuration / 60_000)}min
              </p>
            </div>
