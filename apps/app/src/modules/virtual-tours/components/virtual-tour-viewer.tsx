...
export const VirtualTourViewer: React.FC<VirtualTourViewerProps> = ({
  ...
  const currentTour = useVirtualTourStore(
    useShallow((state) => state.currentTour)
  );
  const currentScene = useVirtualTourStore(
    useShallow((state) => state.currentScene)
  );
  const currentView = useVirtualTourStore(
    useShallow((state) => state.currentView)
  );
  const isPlaying = useVirtualTourStore(useShallow((state) => state.isPlaying));
  const isFullscreen = useVirtualTourStore(
    useShallow((state) => state.isFullscreen)
  );
  ...
  const xrSupported = useVirtualTourStore(
    useShallow((state) => state.xrSupported)
  );
  ...
  const showTourControls = useVirtualTourStore(
    useShallow((state) => state.showTourControls)
  );
  const accessibilityEnabled = useVirtualTourStore(
    useShallow((state) => state.accessibilityEnabled)
  );
  const textToSpeech = useVirtualTourStore(
    useShallow((state) => state.textToSpeech)
  );
  const voiceControl = useVirtualTourStore(
    useShallow((state) => state.voiceControl)
  );
  const highContrast = useVirtualTourStore(
    useShallow((state) => state.highContrast)
  );

  const setCurrentTour = useVirtualTourStore(
    useShallow((state) => state.setCurrentTour)
  );
  const setCurrentScene = useVirtualTourStore(
    useShallow((state) => state.setCurrentScene)
  );
  const setIsPlaying = useVirtualTourStore(
    useShallow((state) => state.setIsPlaying)
  );
  ...
  const setXRSupported = useVirtualTourStore(
    useShallow((state) => state.setXRSupported)
  );
  const setIsMobile = useVirtualTourStore(
    useShallow((state) => state.setIsMobile)
  );
  ...
  useEffect(() => {
    const initializeServices = async () => {
      ...
      // Check XR support
      const xrCapabilities = await import("@/lib/webxr").then((m) =>
        m.getXRCapabilities()
      );
  ...
  useEffect(() => {
    if (isPlaying && tour && tour.scenes.length > 1) {
      const timer = setTimeout(() => {
        nextScene();
      }, 10_000); // 10 seconds per scene

      return () => clearTimeout(timer);
    }
  }, [isPlaying, tour, nextScene]);
  ...
  useEffect(() => {
    if (voiceControl) {
      AccessibilityService.initializeVoiceControl();
  ...
  const handleStartXR = async (mode: "vr" | "ar") => {
    if (!xrSupported) {
      toast.error("XR not supported on this device");
      return;
    }
  ...
  const currentSceneData = tour.scenes.find(
    (s: { id: string | null }) => s.id === currentScene
  );
  const progress =
    currentScene && tour.scenes.length > 1
      ? ((tour.scenes.findIndex((s: { id: string }) => s.id === currentScene) +
          1) /
          tour.scenes.length) *
        100
      : 0;
              ...
              {tour.scenes.length > 1 && (
                <div className="flex items-center gap-2 px-2">
                  <span className="text-sm text-white">
                    {tour.scenes.findIndex(
                      (s: { id: string | null }) => s.id === currentScene
                    ) + 1}{" "}
                    / {tour.scenes.length}
                  </span>
                  <Progress className="h-1 w-20" value={progress} />
                </div>
              )}
          ...
          <div className="-translate-x-1/2 -translate-y-1/2 absolute top-1/2 left-1/2 transform">
            <Card className="bg-orange-600 text-white">
              <CardContent className="p-4 text-center">
                ...
                <p className="text-sm">
                  You're viewing a cached version of this tour
                </p>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex gap-4 text-muted-foreground text-sm">
              ...
              <span>
                ⏱ {Math.round(tour.analytics.averageDuration / 60_000)}min avg
              </span>
