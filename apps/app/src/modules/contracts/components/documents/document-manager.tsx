...
import { useUploadContractDocument, useDeleteContractDocument } from "../../contract.queries";
...
export function DocumentManager({ contract, open, onClose }: DocumentManagerProps) {
	...
	const uploadFile = async (uploadData: FileUploadData, index: number) => {
		try {
			...
			for (let progress = 0; progress <= 100; progress += 10) {
				...
				setUploading((prev) => prev.map((item, i) => (i === index ? { ...item, progress } : item)));
			}
			...
			<Dialog open={open} onOpenChange={onClose}>
				<DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Contract #{contract._id.slice(-8)} - Manage contract documents and files
						</DialogDescription>
					</DialogHeader>
											...
											<div key={index} className="space-y-3 rounded-lg border p-4">
												...
												<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
													<div>
														...
														<Select
															value={upload.type}
															onValueChange={(value: ContractDocument["type"]) => {
																setUploading((prev) =>
																	prev.map((item, i) =>
												...
												{upload.status === "pending" && (
													<div className="flex gap-2">
														<Button
															...
															disabled={!upload.type || upload.status === "pending"}
														>
												...
												<div className="flex items-center gap-4">
													...
													<div className="min-w-0 flex-1">
														<div className="flex items-center gap-2">
															...
															<Badge variant={getDocumentTypeBadge(document.type)}>
																{document.type}
															</Badge>
														</div>
						...
						{selectedDocument?.mimeType?.startsWith("image/") ? (
							...
							<div className="py-8 text-center">
								...
								<p className="mb-4 text-muted-foreground">
									PDF preview not available. Click download to view the document.
								</p>
								<Button onClick={() => selectedDocument && downloadDocument(selectedDocument)}>
									<Download className="mr-2 h-4 w-4" />
									Download PDF
								</Button>
							</div>
