  ...
  const compileTemplate = (template: string, context: Record<string, any>) => {
    try {
      ...
      compiled = compiled.replace(
        /\{\{#if\s+(\w+)\}\}([\s\S]*?)\{\{\/if\}\}/g,
        (_, condition, content) => (context[condition] ? content : "")
      );
      ...
      compiled = compiled.replace(/\{\{(\w+)\}\}/g, (_, variable) =>
        context[variable] !== undefined ? context[variable] : ""
      );
    ...
    <CardContent>
      <div className="grid grid-cols-1 gap-6 text-sm md:grid-cols-2">
        ...
        <div>
          ...
          <p className="mt-1 text-gray-600 text-xs">Loop through array items</p>
      ...
      <div className="rounded-lg bg-blue-50 p-4">
        ...
        <ul className="space-y-1 text-blue-800 text-sm">
          ...
          <li>• Test with different data values to ensure proper formatting</li>
