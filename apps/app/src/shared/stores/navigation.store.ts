// import { entityRelations } from "@/config/entity-config";
...
const initStore: InitStore = {
  ...
  navSheetOpen: null,
  keepMenuOpen:
    typeof window !== "undefined" ? window.innerWidth > 1280 : false, // Auto-open menu on wider screens
        ...
        (set) => ({
          ...
          setFocusView: (status) => {
            set((state) => {
              ...
              if (
                status &&
                document.getElementsByClassName("focus-view-scroll").length
              ) {
