...
} from "@/lib/imado/helpers";
import type { ImadoOptions, UppyBody, UppyMeta } from "@/lib/imado/types";
import { LocalFileStorage } from "@/modules/files/local-file-storage";
import type { UploadUppyProps } from "@/modules/files/upload/upload-uppy";
...
export async function ImadoUppy(
  ...
  if (canUpload) {
    token =
      (await meService.getUploadToken(type, {
        public: isPublic,
  ...
  const imadoUppy = createBaseTusUppy(
    {
      ...
      onBeforeUpload: (files) => {
        ...
        prepareFilesForOffline(files).then((successful) =>
          imadoUppy.emit("complete", { successful, failed: [] })
        );
    ...
    .on("files-added", () => {
      ...
      if (onlineManager.isOnline() && !config.has.imado)
    ...
    .on("complete", (result) => {
      ...
      const mappedResult = successful.map((file) =>
        canUpload
          ? transformUploadedFile(file, token, isPublic)
          : { file, url: file.id }
      );
    ...
    .on("is-online", async () => {
      ...
      const offlineUploadedFiles = imadoUppy
        .getFiles()
        .filter((el) => el.meta.offlineUploaded);
      if (!offlineUploadedFiles.length) return;

      // Get a new upload token
      const imadoToken = await meService.getUploadToken(type, {
        public: isPublic,
        organizationId: opts.organizationId,
      });
      ...
      const retryImadoUppy = createBaseTusUppy(
        uppyOptions,
        imadoToken,
        isPublic
      );
      ...
      const validFiles = offlineUploadedFiles.map((file) => ({
        ...file,
      ...
      retryImadoUppy.upload().then(async (result) => {
        ...
        const successResult = result.successful.map((file) =>
          transformUploadedFile(file, imadoToken, isPublic)
        );
