import type { ContextEntity, Entity } from "@kaa/config";
import type { AppCustomEventMap } from "@/lib/custom-events/app-types";

// import type { Membership } from '@/modules/members/types';
...
export type CustomEventsWithData = {
  [K in keyof CombinedCustomEventMap as CombinedCustomEventMap[K] extends CustomEvent<
    infer DetailData
  >
    ? IfAny<DetailData, never, K>
    : never]: CombinedCustomEventMap[K] extends CustomEvent<infer DetailData>
    ? DetailData
    : never;
};
