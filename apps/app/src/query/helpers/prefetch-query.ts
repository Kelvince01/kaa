...
export async function prefetchQuery<
  T extends UseQueryOptions<any, any, any, any>,
>(options: T): Promise<InferType<T>>;
export async function prefetchQuery<
  T extends UseInfiniteQueryOptions<any, any, any, any>,
>(options: T): Promise<InferType<T>>;
...
export function prefetchQuery(
  ...
  if ("getNextPageParam" in options)
    return queryClient.fetchInfiniteQuery(options); // Infinite query fetch
