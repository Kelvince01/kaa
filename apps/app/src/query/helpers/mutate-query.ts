 ...
 */
export const getQueryItems = <T>(
  prevItems: QueryData<T> | InfiniteQueryData<T>
) => {
  // Check if the data is of type QueryData or InfiniteQueryData and extract items accordingly
  return isQueryData(prevItems)
    ? prevItems.items
    : prevItems.pages.flatMap((page) => page.items);
};
...
export function formatUpdatedData<T>(
  ...
  if (isQueryData(prevData))
    return { total: prevData.total + addToTotal, items: updatedData };

  // Determine the effective limit without modifying the function parameter
  const pageItemsLimit =
    limit ??
    (prevData.pages.length > 1 ? prevData.pages[0]?.items.length : null);
  ...
  if (!pageItemsLimit) {
    return {
      ...
      pages: [
        {
          total: prevData.pages[0]?.total ?? 0 + addToTotal,
          items: updatedData,
        },
      ],
    };
  ...
  return {
    ...
    pages: chunks.map((chunk, index) => ({
      total: prevData.pages[index]?.total ?? 0 + addToTotal,
      items: chunk,
    })),
  };
}
 ...
 */
export const getCancelingRefetchQueries = async <T>(
  exactQueryKey: QueryKey,
  similarQueryKey?: QueryKey
) => {
