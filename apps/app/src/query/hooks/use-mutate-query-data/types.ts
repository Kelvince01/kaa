import type { ContextEntity, Entity, ProductEntity } from "@kaa/config";
import type { InfiniteQueryData, QueryData } from "@/query/types";

export type LimitedEntity = {
  id: string;
  name: string;
  entity: Entity;
...
export type MinimumMembershipInfo = {
  id: string;
...
export type ArbitraryEntityQueryData = Record<
  string,
  EntityData | EntityData[]
>;

export type UseMutateQueryDataReturn = {
  /**
  ...
  updateMembership: {
    ...
    (
      items: ItemData[] | ContextEntityData[],
      entity?: ProductEntity | ContextEntity,
      keyToOperateIn?: string
    ): void;
  };
