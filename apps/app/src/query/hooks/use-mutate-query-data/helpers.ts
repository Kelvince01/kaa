 ...
 */
export const isArbitraryQueryData = (
  data: unknown
): data is ArbitraryEntityQueryData => {
 ...
 */
export const changeInfiniteQueryData = (
  queryKey: QueryKey,
  items: ItemData[],
  ...
  queryClient.setQueryData<InfiniteEntityQueryData>(queryKey, (data) => {
    ...
    const pages = data.pages.map(
      (page: InfiniteEntityQueryData["pages"][number]) => ({
...
const updateArrayItems = <T extends ItemData>(
  ...
  switch (action) {
    ...
    case "update":
      ...
      return items.map(
        (item) => dataItems.find((i) => i.id === item.id) ?? item
      );
    ...
    case "updateMembership": {
      ...
      return items.map((item) => {
        if (item.membership) {
          const updatedMembership = dataItems.find(
            (i) => i.id === item.membership?.id
          );
          return updatedMembership
            ? {
                ...item,
                membership: { ...item.membership, ...updatedMembership },
              }
            : item;
        }
 ...
 */
const updateItem = <T extends ItemData>(
  prevItem: T,
  newItem: T,
  action: QueryDataActions
) => {
