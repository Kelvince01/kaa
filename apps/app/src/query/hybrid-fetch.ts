 ...
 */
export const hybridFetch = async <T>(
  options: FetchQueryOptions<any, any, any, any>,
  refetchIfOnline = true
): Promise<T | undefined> => {
  ...
  try {
    ...
    return (await queryClient.fetchQuery(options)) as T;
  } catch (error) {
 ...
 * @returns Returns query data or undefined.
 */
export const hybridFetchInfinite = async (
  options: FetchInfiniteQueryOptions<any, any, any, any, any>,
  refetchIfOnline = true
) => {
  const { queryKey } = options;
